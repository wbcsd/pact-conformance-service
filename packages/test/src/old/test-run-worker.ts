...
import { getAccessToken, fetchOpenIdTokenEndpoint } from "./authUtils";
...
export class TestRunWorker {
  ...
  async startTestRun(params: TestRunStartParams): Promise<TestRunWithResults> {
    ...
    if (!params.baseUrl || !params.clientId || !params.clientSecret) {
      throw new BadRequestError("Missing required parameters: baseUrl, clientId, and clientSecret are mandatory.");
    }
