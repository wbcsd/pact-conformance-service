...
import { TestDefinition, runTest, runListenerTest } from "./test-definition";
...
export class TestRunWorker {
  ...
  /**
   * @param output - storage backend for test runs and results.
   * @param webhookUrl - public URL of the callback listener that the tested API will post events to.
   *   Used as the CloudEvents `source` for outgoing requests so the API knows where to send replies.
   */
  constructor(
    private output: TestStorage,
  ...
  async startTestRun(params: TestRunStartParams): Promise<TestRunWithResults> {
    ...
    const context = new TestContext(testRun, params, schema, this.webhookUrl);
